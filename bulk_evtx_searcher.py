...
Description:
    Script to carve and search for data in large repositories of log event log
    files where standard review may be tedious. Usecase is to search for item
    like username, domain or IP Address to determine existance and get a copy
    of record. The script will spit out the record containing each search hit
    to an output file configured in run command. Included multiprocess
    capabilities so try to run on a machine with lots of cores for faster
    processing.
...
Instructions:
    ...
    $ python bulk_evtx_searcher.py -s
        "<search,string,comma,delimited>" </source/folder/> </output/file.log>
    $ python bulk_evtx_searcher.py -s
        "USERNAME1,USERNAME2,10.10.10.10,192.182.23.4"
        /cases/data /cases/output.txt
'''
